...
let ticketPriceTotal = runners.reduce((accumulatior,currentValue) =>{
    return accumulatior + currentValue.donation;
},0);
...
// Problem 1
// get the emails for all the runners using .forEach()
let contact= runners.forEach((currentValue)=>{
    console.log (currentValue.email)
    });
    

// Problem 2
// list all the donations larger than 200 by using .filter()
let bigDonation=runners.filter((currentValue)=>{
...
// Problem 3
// change the shirt sizes to lower case using .map
let smallPrint = runners.map((currentValue)=>{
